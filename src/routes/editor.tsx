...
const MIGRATION_FLAG_KEY = "nossahistoria.invite.cloudMigrated";
...
function normalizeDraft(partial?: Partial<InviteDraft> | null): InviteDraft {
  return {
...
function mapRowToDraft(row: SavedInviteRow): InviteDraft {
  return {
...
function rowToSavedVersion(row: SavedInviteRow): SavedVersion {
...
function persistDraftLocally(draft: InviteDraft) {
...
function persistVersionsLocally(versions: SavedVersion[]) {
  if (typeof window === "undefined") return;
  try {
...
async function migrateLocalDraftsToSupabase(userId: string) {
...
function Editor() {
  ...
  const [date, setDate] = useState(initial?.date ?? DEFAULT_DRAFT.date);
  const [time, setTime] = useState(initial?.time ?? DEFAULT_DRAFT.time);
  const [venue, setVenue] = useState(initial?.venue ?? DEFAULT_DRAFT.venue);
  const [city, setCity] = useState(initial?.city ?? DEFAULT_DRAFT.city);
  const [message, setMessage] = useState(initial?.message ?? DEFAULT_DRAFT.message);
  const [tagline, setTagline] = useState(initial?.tagline ?? DEFAULT_DRAFT.tagline);
  const [palette, setPalette] = useState<Palette>(initialPalette);
  const [imageSrc, setImageSrc] = useState<string>(initial?.imageSrc ?? DEFAULT_DRAFT.imageSrc);
    ...
    async ({ hydrateDraft = false }: { hydrateDraft?: boolean } = {}) => {
      ...
      if (autosave) {
        ...
        if (hydrateDraft && !hydratedFromCloud.current) {
          ...
          setBrideName(remoteDraft.brideName);
          setGroomName(remoteDraft.groomName);
          setDate(remoteDraft.date);
          setTime(remoteDraft.time);
          setVenue(remoteDraft.venue);
          setCity(remoteDraft.city);
          setMessage(remoteDraft.message);
          setTagline(remoteDraft.tagline);
          setPalette(PALETTES.find((p) => p.id === remoteDraft.paletteId) ?? PALETTES[0]);
          setImageSrc(remoteDraft.imageSrc);
          persistDraftLocally(remoteDraft);
  ...
  useEffect(() => {
    if (!userId || !autosaveReady) return;
    let cancelled = false;
    ...
    const t = setTimeout(async () => {
      try {
        ...
        setDraftRowId(data?.id ?? draftRowId);
        setAutoStatus("saved");
      } catch (error) {
        if (!cancelled) {
          console.error("Falha ao salvar rascunho na nuvem", error);
          setAutoStatus("idle");
        }
      }
    }, 800);
  ...
  async function saveVersion() {
    if (!userId) {
      toast.error("É preciso estar autenticado para salvar versões.");
      return;
    }
    setSavingVersion(true);
    try {
      const now = new Date();
      const label = `${brideName} & ${groomName} — ${now.toLocaleDateString("pt-BR")}`;
      const payload = buildInvitePayload(draft, userId, { label });
      const { error } = await supabase.from("saved_invites").insert(payload);
      if (error) throw error;
  ...
  async function deleteVersion(id: string) {
    ...
    const next = previous.filter((v) => v.id !== id);
    setVersions(next);
    persistVersionsLocally(next);
    try {
      const { error } = await supabase.from("saved_invites").delete().eq("id", id);
      if (error) throw error;
      await fetchSavedInvites();
