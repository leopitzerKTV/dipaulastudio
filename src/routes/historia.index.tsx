...
const fallbackChapters: Chapter[] = [
  {
    ...
    body: "Foi numa tarde de outono, em uma cafeteria pequena, que tudo começou. Um sorriso bastou.",
    imageUrl: bride,
  },
  {
    id: "f2",
    position: 1,
    storage_path: null,
    date_label: "Maio · 2023",
    title: "O pedido",
    body: "Diante do mar, ao pôr do sol, com o anel na mão e o coração tremendo: 'casa comigo?'",
    imageUrl: proposal,
  },
  {
    id: "f3",
    position: 2,
    storage_path: null,
    date_label: "Maio · 2025",
    title: "O nosso sim",
    body: "Diante de quem amamos, sob arcos de rosas brancas, prometemos uma vida inteira.",
    imageUrl: ceremony,
  },
  {
    id: "f4",
    ...
    date_label: "Maio · 2025",
    title: "A celebração",
    body: "Sparklers no céu, risadas até o amanhecer e a certeza de que isso ficaria para sempre.",
    imageUrl: party,
  },
  {
    id: "f5",
    position: 4,
    storage_path: null,
    date_label: "Junho · 2025",
    title: "Lua de mel",
    body: "Mãos dadas em um cais de madeira, com o mar turquesa lembrando: agora é só o começo.",
    imageUrl: honeymoon,
  },
];
...
function Historia() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "story_chapters" }, () => {
        load();
      })
      .subscribe();
    ...
    <AppShell>
      ...
      <section className="px-6 pt-2 text-center">
        ...
        <h1 className="mt-4 font-display text-4xl text-[var(--cocoa)]">
          A nossa <span className="italic gold-text">história</span>
        </h1>
        <p className="mx-auto mt-2 max-w-xs text-sm text-[var(--cocoa)]/65">
          Cada capítulo, um momento que nos trouxe até aqui.
        </p>
        <Link
          to="/historia/editar"
      ...
      <div className="mt-8 space-y-10 px-5">
          ...
          >
            <div className="relative aspect-[9/16] w-full overflow-hidden rounded-[1.75rem] shadow-[var(--shadow-luxe)] bg-[var(--card)]">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-[var(--ivory)]">
                {c.date_label && (
                  <p className="font-serif-caps text-[10px] opacity-80">{c.date_label}</p>
                )}
                {c.title && <h3 className="mt-1 font-display text-3xl">{c.title}</h3>}
                {c.body && (
                  <p className="mt-2 font-display text-base italic leading-snug opacity-90">
                    {c.body}
                  </p>
                )}
              </div>
        ...
        {!loading && chapters.length === 0 && (
          <p className="text-center font-display italic text-[var(--cocoa)]/60">
            Nenhum capítulo ainda. Toque em “Editar capítulos” para começar.
          </p>
        )}
      </div>

      <Ornament className="mt-12" />
      <p className="mt-3 text-center font-display italic text-[var(--cocoa)]/70">
